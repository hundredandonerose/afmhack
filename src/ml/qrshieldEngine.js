...
import bundledModel from './qrshield_model.json';

let QRS_MODEL = bundledModel;

function setModel(json) {
  if (!json || !Array.isArray(json.features) || !Array.isArray(json.trees)) {
    throw new Error("Invalid QR-Shield model JSON");
  }
  QRS_MODEL = json;
}
  ...
  var KZ_SERVICES = new Set([
    "onay.kz","tha.kz","astanalrt.kz",
    "elicense.kz","enbek.kz","kase.kz","sud.gov.kz","stat.gov.kz",
    "beeline.kz","kcell.kz","tele2.kz","izi.kz","activ.kz","altel.kz","telecom.kz",
    "technodom.kz","sulpak.kz","mechta.kz","magnum.kz","small.kz","flip.kz",
    "meloman.kz","wildberries.kz","ozon.kz","alser.kz","arbuz.kz",
    "krisha.kz","kolesa.kz","market.kz","aviata.kz","chocotravel.com","airastana.com",
    "chocofood.kz","glovoapp.com","wolt.com","2gis.kz","yandex.kz","ticketon.kz",
    "sxodim.com","biletmania.kz"
  ]);
...
function getModelInfo() {
  return {
    features: QRS_MODEL.features.length,
    trees: QRS_MODEL.trees.length,
    nodes: QRS_MODEL.trees.reduce((sum, tree) => sum + tree.length, 0),
    lr: QRS_MODEL.lr,
    f0: QRS_MODEL.f0,
  };
}

export { assess, setModel, getModelInfo };
